...
setMethod("group",
		...
		else stop("Unknown scale for merge criteria")
...
setMethod("print",
		...
		for (j in 1:ntrts) {
		  ...
		  n  <- n + nj
		  write(paste("Sample size in treatment", x@model@trts[j], ":", nj), file="")
		}
		write(paste("Total sample size (excluding missing data) : ", n), file="")
		print(x@model, x, estimate, cov, test, ...)

		if (test)
		  write("Note: The pvalues are not adjusted for multiple testing.",file="")
	    }
)
...
# 1. estimate the treatmente effect and do the permutation test
